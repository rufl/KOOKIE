...
- Native-safe replay sidecars now restore player inventory, equipment and broad-phase geometry through `LoopbackSession`; JVM/native coverage verifies the sidecar. Combining sidecar state with the atomic replay checkpoint payload remains next. Replay checkpoints retain 320 bounded values and restore enemy combat, AI, spatial and projectile state while clearing transient impact presentation/audio queues.
