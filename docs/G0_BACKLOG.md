...
- Replay bundle files now persist primary checkpoint plus sidecar state; `BoundedReplayPresentationHistory` round-trips ordered transient presentation events, and deterministic shotgun geometry exposes bounded radial pellet offsets. JVM/native coverage verifies both paths. Crash-durable save publication remains blocked on host filesystem primitives.
