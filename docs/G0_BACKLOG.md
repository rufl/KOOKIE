...
- Native-safe replay sidecars restore player inventory, equipment and broad-phase geometry through `LoopbackSession`; `appendReplayCheckpointBundle`/`applyReplayCheckpointBundle` now bind those sidecars to the same bounded timeline checkpoint and restore atomically, with JVM/native coverage. Replay checkpoints retain 320 bounded primary values and restore enemy combat, AI, spatial and projectile state while clearing transient impact presentation/audio queues.
