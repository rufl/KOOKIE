...
- Progression modifiers now feed authoritative combat: skill ranks contribute bounded percentage damage, statuses contribute bounded flat/percentage damage, status expiry refreshes player modifiers each fixed step, and enemy combat accepts the same modifier path; JVM/native coverage verifies enhanced and expired player damage plus enemy damage.
