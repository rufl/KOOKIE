...
This is the active bounded implementation sequence after the initial research commit.
...
- JVM/native regression smoke and named resource-token test.

## Next batch

1. Add a tiny native adapter registry that stores SDL-owned pointers behind checked integer tokens. No gameplay ownership, raw pointers, callbacks or bulk buffers.
2. Prove SDL window creation and teardown from the native ELF using the repository's isolated-display wrapper.
3. Poll window events and record focus/resize state through scalar adapter functions. Keep the Kof loop authoritative.
