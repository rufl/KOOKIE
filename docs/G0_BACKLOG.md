...
- Authoritative player weapon presentations now automatically enter bounded replay history; deterministic shotgun geometry exposes target-order starts while preserving native-compatible zero-spread behavior. JVM/native coverage verifies both paths. Crash-durable saves and DRI3 screenshot validation remain blocked by host capabilities.
