...
- Replay bundle state now persists through `BoundedReplayBundleFileStore`, round-tripping primary checkpoints plus inventory/equipment/broad-phase sidecars in one bounded file with JVM/native coverage. Crash-durable save publication remains blocked until the host `File` API exposes atomic replacement, flush/sync, and failure-injection hooks.
