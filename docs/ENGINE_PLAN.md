...
- Inventory/equipment/stash/crafting use validation + reservation + atomic commit. `BoundedItemInventory.purchaseFrom` now atomically transfers item quantity and currency, and `BoundedEquipmentLoadout` validates ownership, prevents duplicate slot assignment, supports bounded unequip, and persists slot/item ownership; equipment, crafting and multi-operation reservations remain required.
