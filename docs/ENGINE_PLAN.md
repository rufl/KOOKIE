...
- Replay stores engine/content version, initial snapshot/seed, tick commands and checkpoints/hash diagnostics. Replay checkpoints now carry bounded state snapshots; seek exposes selected checkpoint state, and `LoopbackSession.encodeReplayCheckpoint`/`applyReplayCheckpoint` restore tick, movement, prediction, snapshot, player combat, skill resources/cooldowns, statuses, enemy combat actors, AI state, spatial positions, spatial projectiles, encounter budgets and encounter links before staging commands. Bounded player inventory/equipment and broad-phase geometry now have standalone replay codecs; checkpoint integration remains required.
