...
- Replay stores engine/content version, initial snapshot/seed, tick commands and checkpoints/hash diagnostics. `BoundedReplayWireCodec` and `BoundedReplayFileStore` now persist bounded engine/content metadata, seed, signed tick commands, ordered checkpoints, hash diagnostics and initial snapshots with corruption rejection; bounded seek selects a checkpoint and `BoundedReplayTimeline.stageSeekCommands` stages commands through the target tick for caller-owned simulation re-execution. Timestamped presentation events are not sufficient.
