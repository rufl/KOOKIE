...
- Replay stores engine/content version, initial snapshot/seed, tick commands and checkpoints/hash diagnostics. Replay checkpoints now carry bounded state snapshots; seek exposes selected checkpoint state, `LoopbackSession.applyReplayCheckpoint` restores bounded tick/position/snapshot state, and `queueReplayCommands` stages commands from that checkpoint into authoritative fixed-step execution. Full gameplay-state restoration remains required.
