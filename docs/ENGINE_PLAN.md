...
- Replay stores engine/content version, initial snapshot/seed, tick commands and checkpoints/hash diagnostics. `BoundedReplayBundleFileStore` persists primary checkpoint values and replay sidecars in one bounded wire file; `BoundedReplayPresentationHistory` preserves ordered transient presentation events as explicit replay state; `LoopbackSession.appendReplayCheckpointBundle`/`applyReplayCheckpointBundle` bind inventory, equipment and broad-phase state to one timeline checkpoint. Deterministic integer shotgun pellet geometry now exposes bounded radial offsets per pellet. Crash-durable save publication remains blocked on host `File` APIs for atomic rename, flush and directory sync.
