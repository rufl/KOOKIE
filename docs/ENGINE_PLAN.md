...
- Snapshot at a defined tick boundary; stage file, validate/checksum, flush/sync, atomic replacement and directory-durability policy appropriate to the target. Recover from interrupted writes; corruption detection is not authentication.
- Migrations operate on schemas, never raw slots/pointers. Unknown/newer required sections fail with a useful error and leave the old save intact.
- Replay stores engine/content version, initial snapshot/seed, tick commands and checkpoints/hash diagnostics. Seeking restores a checkpoint then re-simulates. Timestamped presentation events are not sufficient.
