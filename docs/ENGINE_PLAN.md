...
- Skill definitions declare prerequisites/rank caps/costs/cooldowns/tags; `BoundedSkillDefinitionStore` and `BoundedSkillProgression` now provide bounded learn/rank/experience progression with skill v1 save section persistence.
- Status definitions specify refresh/replace/add-stack rules, duration cap and ticking schedule. `BoundedStatusDefinitionStore` and `BoundedStatusEffects` now refresh duration, cap additive stacks, expire deterministically and persist status v1 state. Death/reset/load clears or retains effects intentionally.
