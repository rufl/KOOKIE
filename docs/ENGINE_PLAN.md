...
- Replay presentation capture is now automatic for authoritative player weapon events; `BoundedReplayPresentationHistory` stores the ordered bounded state. `CombatWorld.shotgunTargetStartIndex` exposes deterministic geometry-derived target ordering while preserving the legacy zero-spread path for native compatibility. Crash-durable save publication and DRI3 presentation remain host-capability blockers.
