...
- Skill definitions declare prerequisites/rank caps/costs/cooldowns/tags; `BoundedSkillDefinitionStore` and `BoundedSkillProgression` now provide bounded learn/rank/experience progression with skill v1 save section persistence, and `LoopbackSession` exposes authoritative per-player skill learning/ranking APIs whose configured damage percentages feed player combat.
- Status definitions specify refresh/replace/add-stack rules, duration cap and ticking schedule. `BoundedStatusDefinitionStore` and `BoundedStatusEffects` now refresh duration, cap additive stacks, expire deterministically and persist status v1 state; `LoopbackSession` ticks and exposes authoritative per-player statuses each fixed step, with configured flat/percentage damage modifiers applied to player combat. Enemy combat accepts the same bounded modifier path. Death/reset/load clears or retains effects intentionally.
