...
- Replay stores engine/content version, initial snapshot/seed, tick commands and checkpoints/hash diagnostics. `BoundedReplayWireCodec` and `BoundedReplayFileStore` now persist bounded engine/content metadata, seed, signed tick commands, ordered checkpoints and hash diagnostics with corruption rejection; initial snapshot and seek/re-simulation still require implementation. Timestamped presentation events are not sufficient.
