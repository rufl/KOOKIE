...
- Inventory/equipment/stash/crafting use validation + reservation + atomic commit. `BoundedItemInventory.purchaseFrom` now atomically transfers item quantity and currency, `BoundedEquipmentLoadout` validates ownership, prevents duplicate slot assignment, supports bounded unequip and persists slot/item ownership, and `BoundedCraftingSystem` atomically consumes bounded ingredients/currency into a rolled output; crafted inventory/roll/currency state round-trips through the existing save sections. Multi-operation reservations remain required.
...
- Migrations operate on schemas, never raw slots/pointers. `BoundedSaveSections`, `BoundedSaveSchemaWireCodec`, `BoundedSaveSchemaFileStore` and `BoundedSaveMigrationGate` now persist progression v1, item v1, quest v1, world v2, RNG v2, currency v1, rolled-item v1 and equipment v1 sections, transforming prior world/RNG versions with default fields; typed bounded item ownership/quantity, quest state/progress, atomic inventory currency/item transactions, rolled item fields and equipment slot ownership round-trip through the schema wire; unsupported/newer sections fail with the old save left intact. Future section-version transforms remain required.
- Replay stores engine/content version, initial snapshot/seed, tick commands and checkpoints/hash diagnostics. Replay checkpoints now carry bounded state snapshots; seek exposes the selected checkpoint state and stages commands through the target tick, and `LoopbackSession.queueReplayCommands` feeds those commands into authoritative fixed-step execution. Full game-state restoration and caller-selected checkpoint application remain required.
