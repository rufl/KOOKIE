...
- Replay stores engine/content version, initial snapshot/seed, tick commands and checkpoints/hash diagnostics. `BoundedReplayBundleFileStore` now persists primary checkpoint values and replay sidecars in one bounded wire file; `LoopbackSession.appendReplayCheckpointBundle`/`applyReplayCheckpointBundle` bind inventory, equipment and broad-phase state to one timeline checkpoint. Crash-durable save publication remains blocked on host `File` APIs for atomic rename, flush and directory sync.
