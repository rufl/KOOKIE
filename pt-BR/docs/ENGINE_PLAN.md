...
As sondas pequenas de núcleo/escalar/importação/IO do G0 estão **parcialmente pesquisadas**, não completas. Os experimentos precisos em RESEARCH_PROBES e COURSE_PROBES incluem sucessos e falhas nativas não resolvidas. Ainda não há reparo do compilador, janela SDL, adaptador de GPU, prova de temporização sustentada ou prova de memória.### Hipóteses iniciais de desempenho, não números alcançados
